...
from .models import User, Physic, Preferences, Meal, TestList, SignUpToken, ratings,workout, workout_storage
...
class ProfileView(APIView):
    ...
    def put(self,request,*args, **kwargs):
        ...
        user.save()
        return Response({"msg" : "your account has been updated !"})
...
class MealView(APIView):
    ...
    def get(self,request):
        ...
        s = MealSerializer(meals_list[:50],many=True)
...
class OpenAISport(APIView):
    def post(self,request):
        ...
        if(len(wowo) == 0):
            new_workout = workout_storage.objects.filter(numberOfWorkout  = nbr_workout, goal = goal_string).first()
            wowo2 = new_workout.workout_list
            workout_name = new_workout.workout_names
            workout_profile = workout.objects.filter(user=user).update(workout_names = workout_name,workout_list=wowo2)
            print(wowo2)
            print('it crashed')
        else:
            ...
            workout_storage1 = workout_storage.objects.create(workout_names = workout_names,workout_list=wowo2,numberOfWorkout = nbr_workout, goal = goal_string)
            workout_storage1.save()
